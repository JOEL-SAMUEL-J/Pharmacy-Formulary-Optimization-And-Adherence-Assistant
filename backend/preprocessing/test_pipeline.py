...
class PipelineTest(unittest.TestCase):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for name,schema in SCHEMAS.items():
                with (raw/schema.filename).open("w",newline="",encoding="utf-8") as handle:
                    ...
                    writer.writeheader()
                    row=self.row(name)
                    if name == "insulin_beneficiary_cost": row["tier"] = "."
                    writer.writerow(row)
    ...
    def test_same_plan_in_two_counties_is_not_duplicate(self):
        schema=SCHEMAS["plan_information"]
        first=self.row("plan_information")
        second={**first,"county_code":"06003"}
        from preprocessing.validation import grain_digest
        self.assertNotEqual(grain_digest(first,schema.grain),grain_digest(second,schema.grain))
